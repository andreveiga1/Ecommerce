...
# Mapeamento: apelido curto → nome real da tabela no mock
_TABLE_MOCK_MAP = {
    "vendas_temporais": "vendas_temporais",
    "clientes_segmentacao": "clientes_segmentacao",
    "precos_competitividade": "precos_competitividade",
}

# Guarda se o banco está disponível (checado uma vez por sessão)
if "db_available" not in st.session_state:
    st.session_state.db_available = None  # None = ainda não testado


@st.cache_resource
def get_connection():
    """Return a reusable psycopg2 connection (or None if unavailable)."""
...
def load_table(table_key: str) -> pd.DataFrame:
    """
    Carrega uma tabela do banco de dados ou, se indisponível,
    retorna dados mockados automaticamente.

    table_key deve ser um dos valores de _TABLE_MOCK_MAP:
      'vendas_temporais', 'clientes_segmentacao', 'precos_competitividade'
    """
    sql_map = {
        "vendas_temporais": "SELECT * FROM public_gold_sales.vendas_temporais",
        "clientes_segmentacao": "SELECT * FROM public_gold.clientes_segmentacao",
        "precos_competitividade": "SELECT * FROM public_gold.precos_competitividade",
    }

    conn = get_connection()
    if conn is not None:
        try:
            df = pd.read_sql(sql_map[table_key], conn)
            st.session_state.db_available = True
            return df
        except Exception:
            # Conexão estava viva mas query falhou → tenta reconectar uma vez
    ...
    # Banco indisponível → dados mockados
    st.session_state.db_available = False
    return get_mock_data(table_key)
...
if page == "Vendas":
    ...
    # Load data
    df_vendas = load_table("vendas_temporais")
...
# ══════════════════════════════════════════════
    ...
    df_clientes = load_table("clientes_segmentacao")
...
# ══════════════════════════════════════════════
    ...
    df_pricing = load_table("precos_competitividade")
